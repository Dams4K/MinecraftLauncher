extends MCTweaker
class_name FabricTweaker

const MAVEN_URL = "https://maven.fabricmc.net"
const API_URL = "https://meta.fabricmc.net/"
const LOADER_PATH = "v2/versions/loader/%s/%s"

@export var loader_version: StringName

var loader_data: Dictionary = {}

func setup(downloader: Requests):
	var url = API_URL.path_join(LOADER_PATH % [minecraft_version, loader_version])
	loader_data = (await downloader.do_get(url)).json()
